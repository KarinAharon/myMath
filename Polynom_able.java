 ...
 * where: a_1, a_2 ... a_n are real numbers and b_1 smaller than b_2.. smaller than b_n bigger or equal 0 are none negative integers (naturals)
 ...
 * 3.5 double root(double x0, double x1, double eps) // assuming (f(x0)*f(x1)smaller or equal 0, returns f(x2) such that:
 *													//	(i) x0 smaller or equal x2 smaller or equal x1  (ii) {f(x2) smaller than eps
...
public interface Polynom_able extends cont_function{
	...
	/**
	 * Compute a value x' (x0 smaller or equal x'smaller or equal x1) for with |f(x')| smaller than eps
	 * assuming (f(x0)*f(x1) smaller or equal 0, returns f(x2) such that:
	 * (i) x0 smaller or equal x2 smaller or equal x2 (ii) f(x2) smaller than eps
